...
exports.getRekaiSystemPrompt = void 0;
const formatVehicleInfo = (vehicle) => {
    if (!vehicle)
        return 'Araç bilgisi mevcut değil.';
    return [
        `Marka: ${vehicle.brand}`,
...
const getRekaiSystemPrompt = (vehicle) => {
    const parts = [
    ...
    if ((vehicle === null || vehicle === void 0 ? void 0 : vehicle.fuelType) === 'Dizel') {
        parts.push('- Dizel motor bakımı için partikül filtresi ve enjektör temizliği önerilerini hatırlat.');
    }
    return parts.join('\n');
};
exports.getRekaiSystemPrompt = getRekaiSystemPrompt;
