...
const TirePartsScreen = () => {
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (location) {
        ...
        console.log('📍 TirePartsScreen: Konum alındı:', location);
      } else {
        console.log('📍 TirePartsScreen: Konum alınamadı');
      }
    } catch (error) {
      console.error('TirePartsScreen: Konum alma hatası:', error);
    } finally {
  ...
  const loadVehicleData = async () => {
    try {
      ...
      setVehicleInfo({
        ...
        engine: data.vehicleType
      });
      setTireSize(data.tireSize);
      setTireBrand(data.tireBrand);
      setTireModel(data.tireModel);
    } catch (error) {
  ...
  const handleRequestParts = async () => {
    ...
    try {
      ...
      console.error('Parça talebi hatası:', error);
