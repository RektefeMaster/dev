...
export const sortMechanicsByDistance = (
  ...
  if (!mechanics || !Array.isArray(mechanics)) {
    console.log('⚠️ sortMechanicsByDistance: Invalid mechanics array');
    return [];
  }

  console.log(`🔍 sortMechanicsByDistance: Processing ${mechanics.length} mechanics`);
  ...
  const mechanicsWithDistance = mechanics
    .map((mechanic, index) => {
      ...
      if (isValid) {
        ...
        console.log(`🔍 ${mechanic.name}: Valid coords (${coordinates.latitude}, ${coordinates.longitude}) -> ${distance.toFixed(2)} km`);
        return {
          ...mechanic,
          distance: distance,
        ...
        // Invalid coordinates - log the issue
        console.log(`⚠️ ${mechanic.name}: Invalid coordinates - raw:`, raw, 'normalized:', coordinates);
      }
  ...
  console.log('🔍 Final sorted mechanics (first 3):', sortedMechanics.slice(0, 3).map(m => ({
    name: m.name,
    distance: m.distance,
