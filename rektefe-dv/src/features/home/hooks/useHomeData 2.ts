...
import { clearAuthData } from '@/shared/utils/common';
import { apiService } from '@/shared/services/api';
import { getRealUserLocation, getFallbackUserLocation, sortMechanicsByDistance, formatDistance } from '@/shared/utils/distanceCalculator';
...
export const useHomeData = () => {
  ...
  const fetchData = async (token: string, userId: string) => {
    setLoading(true);
      setError(null);
    try {
      // Önce kullanıcı konumunu al
      const currentLocation = await fetchUserLocation();
      
      // Konum alındıktan sonra en yakın ustayı hesapla
      await fetchNearestMechanic(token, currentLocation);
      
      // Diğer veri çekme işlemlerini paralel olarak başlat
      await Promise.all([
  ...
  const fetchUserLocation = async () => {
    try {
      ...
      if (location) {
        ...
        return location; // Konum bilgisini döndür
      } else {
        ...
        return fallbackLocation; // Fallback konum bilgisini döndür
      }
    } catch (error) {
  ...
  const fetchNearestMechanic = async (token: string, currentUserLocation?: any) => {
    try {
      // Tüm ustaları getir - MechanicSearchScreen'deki endpoint'i kullan
      const response = await apiService.getMechanics();
      
      if (!response.success) {
        throw new Error('Ustalar getirilemedi');
      }
      ...
      if (data && data.success && data.data && data.data.length > 0) {
        ...
        const mechanicsWithDetails = await Promise.all(
          mechanics.map(async (mech: any) => { // Tüm ustalar için detay al
            try {
              const detailsResponse = await apiService.getMechanicDetails(mech._id);
        ...
        if (locationToUse) {
          console.log('🔍 User location:', locationToUse);
          console.log('🔍 Total mechanics before sorting:', mechanicsWithDetails.length);
          console.log('🔍 Sorting mechanics by distance...');
          
          // Her ustanın koordinat bilgilerini logla
          console.log('🔍 Mechanics coordinates check:');
          mechanicsWithDetails.slice(0, 3).forEach((m, index) => {
          ...
          sortedMechanics = sortMechanicsByDistance(mechanicsWithDetails, locationToUse);
          console.log('🔍 Sorted mechanics (first 5):', sortedMechanics.slice(0, 5).map(m => ({
            name: m.name,
            distance: m.distance,
