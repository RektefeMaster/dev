...
export default function ElectricalJobsScreen() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await apiService.getElectricalJobs(status);

      if (response.success && response.data) {
        let filteredJobs = response.data;
        if (activeTab === 'active') {
          filteredJobs = response.data.filter((job: ElectricalJob) => 
            job.status !== 'completed' && job.status !== 'cancelled'
          );
        }
        setJobs(filteredJobs);
      } else {
        Alert.alert('Uyarı', response.message || 'Elektrik işleri yüklenirken bir sorun oluştu');
      }
    } catch (error: any) {
      console.error('Fetch electrical jobs error:', error);
      if (error.response?.status === 401) {
        Alert.alert('Oturum Hatası', 'Oturumunuz sona ermiş. Lütfen tekrar giriş yapın.');
      } else {
        Alert.alert('Hata', 'Elektrik işleri yüklenirken bir hata oluştu. Lütfen tekrar deneyin.');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const renderJobCard = ({ item }: { item: ElectricalJob }) => {
    const urgencyBadgeStyle = getUrgencyLevelBadgeStyle(item.electricalInfo.urgencyLevel);
    ...
    return (
      ...
      >
        ...
        <View style={styles.jobDetails}>
          <View style={styles.electricalInfo}>
            <Ionicons name={getElectricalSystemIcon(item.electricalInfo.systemType) as any} size={16} color={theme.colors.primary.main} />
            <Text style={styles.electricalText}>
              {translateElectricalSystemType(item.electricalInfo.systemType)} - {translateElectricalProblemType(item.electricalInfo.problemType)}
            </Text>
          </View>

          <View style={styles.badgesContainer}>
            {item.electricalInfo.urgencyLevel === 'acil' && (
              <View style={[styles.urgencyBadge, urgencyBadgeStyle]}>
                <Ionicons name={getUrgencyLevelIcon(item.electricalInfo.urgencyLevel) as any} size={12} color={urgencyBadgeStyle.color} />
                <Text style={[styles.badgeText, { color: urgencyBadgeStyle.color }]}>Acil</Text>
              </View>
            )}
            {item.electricalInfo.isRecurring && (
              <View style={[styles.recurringBadge, recurringBadgeStyle]}>
                <Ionicons name="repeat" size={12} color={recurringBadgeStyle.color} />
                <Text style={[styles.badgeText, { color: recurringBadgeStyle.color }]}>Tekrarlayan</Text>
              </View>
            )}
          </View>

          <Text style={styles.descriptionText} numberOfLines={2}>
            {item.electricalInfo.description}
          </Text>

          {item.electricalInfo.photos && item.electricalInfo.photos.length > 0 && (
            <View style={styles.photosPreview}>
              {item.electricalInfo.photos.slice(0, 3).map((photo, index) => (
                <Image key={index} source={{ uri: photo }} style={styles.photoPreview} />
              ))}
              {item.electricalInfo.photos.length > 3 && (
                <View style={styles.morePhotos}>
                  <Text style={styles.morePhotosText}>+{item.electricalInfo.photos.length - 3}</Text>
                </View>
              )}
            </View>
          )}
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      <Ionicons name="flash-outline" size={64} color={theme.colors.text.secondary} />
      <Text style={styles.emptyTitle}>
        {activeTab === 'active' ? 'Aktif İş Yok' : 'Tamamlanan İş Yok'}
      </Text>
      <Text style={styles.emptyText}>
        {activeTab === 'active'
          ? 'Henüz aktif elektrik işiniz bulunmuyor.'
          : 'Henüz tamamlanan elektrik işiniz bulunmuyor.'}
      </Text>
      {activeTab === 'active' && (
        <TouchableOpacity
          style={styles.createButton}
          onPress={() => navigation.navigate('CreateElectricalJob' as never)}
        >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Elektrik İşleri</Text>
        <TouchableOpacity onPress={() => navigation.navigate('CreateElectricalJob' as never)}>
