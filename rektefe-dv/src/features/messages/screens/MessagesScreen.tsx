...
const MessagesScreen = ({ navigation }: any) => {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    // Search query yoksa tüm conversation'ları göster
    if (!searchQuery || searchQuery.trim() === '') {
      return true;
    }
    
    const isValid = conv.otherParticipant.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Ionicons name="add" size={24} color={themeColors.primary.main} />
        </TouchableOpacity>
      </View>
