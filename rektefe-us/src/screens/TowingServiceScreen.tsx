...
export default function TowingServiceScreen() {
  ...
  const fetchTowingJobs = async () => {
    try {
      ...
      console.error('Çekici işleri yüklenemedi:', error);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.emergencyFilterRow}>
          <TouchableOpacity
            style={[styles.emergencyFilterBtn, onlyEmergency && { backgroundColor: colors.error.main, borderColor: colors.error.main }]}
            onPress={() => setOnlyEmergency(v => !v)}
          >
            <Ionicons name="warning" size={16} color={onlyEmergency ? '#fff' : colors.error.main} />
